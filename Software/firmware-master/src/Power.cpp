 ...
 */
class AnalogBatteryLevel : public HasBatteryLevel, public Adafruit_MAX17048
{
    /**
     * Battery state of charge, from 0 to 100 or -1 for unknown
     */
    ...
    {
#ifdef HAS_MAX17048
        float mybattery_SOC = cellPercent() * 1.12;
        return clamp((int)(mybattery_SOC), 0, 100);
#endif
...
#ifdef HAS_MAX17048
        float battery_Voltage = cellVoltage();
...
#define BATTERY_SENSE_SAMPLES \
    ...
    {
#ifdef HAS_MAX17048
        uint16_t ICversion = getICversion();
        uint8_t ChipID = getChipID();
        // LOG_INFO("ICversion = %d  ChipID = %d\n",ICversion,ChipID);
        return ((ICversion != 0xFFFF) && (ChipID != 0xFF));
#endif
...
#ifdef HAS_MAX17048
        return nrfx_power_usbstatus_get();
#endif
...
int32_t Power::runOnce()
{
...
#if defined(HAS_PMU) && !(HAS_MAX17048)
    ...
    return (statusHandler && statusHandler->isInitialized()) ? (1000 * 10) : RUN_SAME;
}
